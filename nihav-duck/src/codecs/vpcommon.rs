...
pub fn rescale_prob(prob: u8, weights: &[i16; 2], maxval: i32) -> u8 {
    (((i32::from(prob) * i32::from(weights[0]) + 128) >> 8) + i32::from(weights[1]))
        .min(maxval)
        .max(1) as u8
}
...
pub fn vp_idct(coeffs: &mut [i16; 64]) {
    ...
    for (src, dst) in coeffs.chunks(8).zip(tmp.chunks_exact_mut(8)) {
        idct_step!(
            src[0], src[1], src[2], src[3], src[4], src[5], src[6], src[7], dst[0], dst[1], dst[2],
            dst[3], dst[4], dst[5], dst[6], dst[7], 0, 0, i32
        );
    }
    let src = &tmp;
    let dst = coeffs;
...
pub fn vp_add_block_ilace(
    ...
    for y in 0..8 {
        for x in 0..8 {
            frm.data[off + x] = (coeffs[x + y * 8] + i16::from(frm.data[off + x]))
                .min(255)
                .max(0) as u8;
        }
        off += frm.stride[plane] * 2;
