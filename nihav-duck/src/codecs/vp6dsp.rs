...
pub fn get_block(
    ...
    if (sx - 2 < 0) || (sx + 8 + 2 > (w as isize)) || (sy - 2 < 0) || (sy + 8 + 2 > (h as isize)) {
        edge_emu(
            &src,
            sx - 2,
            sy - 2,
        ...
        let src = &sbuf[saddr..];
        for (dline, sline) in dst
            .chunks_exact_mut(dstride)
...
macro_rules! mc_filter {
    ...
    (bicubic; $src: expr, $off: expr, $step: expr, $coeffs: expr) => {
        ((i32::from($src[$off - $step]) * i32::from($coeffs[0])
            ...
            + i32::from($src[$off + $step * 2]) * i32::from($coeffs[3])
            + 64)
            >> 7)
            .min(255)
            .max(0) as u8
...
//#[allow(snake_case)]
pub fn mc_bilinear<const SOFF: usize, const SSTRIDE: usize>(
    dst: &mut [u8],
    dstride: usize,
    src: &[u8],
    mx: u16,
    ...
    if my == 0 {
        for l in (0..dst.len()).step_by(dstride).take(8) {
            let src_data = &src[soff..(soff + 9)];
            for i in 0..8 {
                dst[l + i] = mc_filter!(bilinear; src_data[i], src_data[i + 1], mx);
            }
            soff += SSTRIDE;
        }
    } else if mx == 0 {
        for l in (0..dst.len()).step_by(dstride).take(8) {
            ...
            for i in 0..8 {
                dst[l + i] = mc_filter!(bilinear; src_data[i], src_data[i + SSTRIDE], my);
            }
            soff += SSTRIDE;
        }
    } else {
...

        {
            let src_data = &src[soff..(soff + 9)];
            for i in 0..8 {
                tmp[i] = mc_filter!(bilinear; src_data[i], src_data[i + 1], mx);
            }
            soff += SSTRIDE;
        }
        ...
        for l in (0..dst.len()).step_by(dstride).take(8) {
            ...
            for i in 0..8 {
                ...
                dst[l + i] = mc_filter!(bilinear; tmp[i], cur, my);
                tmp[i] = cur;
            }
            soff += SSTRIDE;
        }
