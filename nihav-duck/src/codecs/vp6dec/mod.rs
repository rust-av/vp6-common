...
impl VP56Parser for VP6BR {
    ...
    fn decode_mv(&self, bc: &mut BoolCoder, model: &VP56MVModel) -> i16 {
        let val = if !bc.read_prob(model.nz_prob) {
            // short vector
            vp_tree!(
                bc,
                model.tree_probs[0],
    ...
    fn decode_block(
        ...
        let token = decode_token_bc(
            bc,
            &model.dc_token_probs[0][dc_mode],
            model.dc_value_probs[5],
            true,
            true,
        );
        ...
        while idx < 64 {
            ...
            let token = decode_token_bc(
                bc,
                &model.ac_val_probs[ac_mode][ac_band],
                model.ac_val_probs[ac_mode][ac_band][5],
                false,
                has_nnz,
            );
            if token == 42 {
            ...
            if val == 0 {
                idx +=
                    decode_zero_run_bc(bc, &vp6model.zero_run_probs[if idx >= 7 { 1 } else { 0 }]);
                validate!(idx <= 64);
            }
        }
    ...
    fn decode_block_huff(
        ...
        let plane = if (fstate.plane == 0) || (fstate.plane == 3) {
            0
        } else {
            1
        };
        ...
        while idx < 64 {
            ...
            if val == 0 {
                idx +=
                    decode_zero_run_huff(br, &model.zero_run_tree[if idx >= 7 { 1 } else { 0 }])?;
                validate!(idx <= 64);
            }
        }
    ...
    if has_nnz && !bc.read_prob(probs[0]) {
        ...
        vp_tree!(
            ...
            vp_tree!(
                bc,
                probs[3],
                vp_tree!(bc, probs[4], 2, vp_tree!(bc, prob34, 3, 4)),
                TOKEN_LARGE
            )
        )
    }
...
impl NAOptionHandler for VP6Decoder {
    ...
    fn query_option_value(&self, _name: &str) -> Option<NAValue> {
        None
    }
}
