...
impl VP56DCPred {
    ...
    fn reset(&mut self) {
        self.update_row();
        for el in self.ref_y.iter_mut().skip(1) {
            *el = INVALID_REF;
        }
        for el in self.ref_c.iter_mut().skip(1) {
            *el = INVALID_REF;
        }
...
impl VP6Encoder {
            ...
            for mb_x in 0..self.mb_w {
                ...
                if !is_intra {
                    estimate_mb_type(
                        mb_type,
                        last_mbt,
                        ((self.fenc.num_mv[mb_idx] + 1) % 3) as usize,
                        &mut self.stats,
                    );
                    last_mbt = mb_type;
                    match mb_type {
                        ...
                        VPMBType::InterFourMV => {
                            for (&sub_type, &mv) in self.fenc.fmv_sub[mb_idx]
                                .iter()
                                .zip(self.fenc.coded_mv[mb_idx].iter())
                            {
                ...
                for i in 0..4 {
                    ...
                    estimate_block(
                        &mb.coeffs[i],
                        dc_mode,
                        &mut self.stats.coeff_models[0],
                        &mut self.stats.vp6models,
                        &self.models.vp6models.zigzag,
                    );
                }
    ...
    fn prepare_huff_models(&mut self) {
        for i in 0..2 {
            self.models.vp6huff.dc_token_tree[i]
                .build_codes(&self.models.coeff_models[i].dc_value_probs);
        }
        for i in 0..2 {
            for mode in 0..3 {
                for band in 0..6 {
                    self.models.vp6huff.ac_token_tree[i][mode][band]
                        .build_codes(&self.models.coeff_models[i].ac_val_probs[mode][band]);
                }
            }
        }
        for i in 0..2 {
            self.models.vp6huff.zero_run_tree[i]
                .build_codes_zero_run(&self.models.vp6models.zero_run_probs[i]);
        }
    }
    ...
    fn encode_inter(&mut self, bw: &mut ByteWriter, quant: usize) -> EncoderResult<bool> {
        ...
        let lambda = if self.force_q.is_some() {
            1.0
        } else {
            self.ratectl.lambda
        };
        self.fenc.select_inter_blocks(
            self.last_frame.get_vbuf().unwrap(),
            self.mc_buf.clone(),
            !self.last_gold,
            lambda,
        );
        ...
        encode_mode_prob_models(
            &mut bc,
            &mut self.models,
            &self.pmodels,
            &self.stats.mbtype_models,
        )?;
        ...
        for _mb_y in 0..self.mb_h {
            ...
            for mb_x in 0..self.mb_w {
                ...
                for i in 0..4 {
                    ...
                    match writer {
                        VP6Writer::BoolCoder(ref mut bc) => encode_block(
                            bc,
                            &mb.coeffs[i],
                            dc_mode,
                            &self.models.coeff_models[0],
                            &self.models.vp6models,
                        )?,
                        VP6Writer::Huffman(ref mut huff) => encode_block_huff(
                            huff,
                            &self.models.vp6models.zigzag,
                            &mb.coeffs[i],
                            0,
    ...
    fn negotiate_format(&self, encinfo: &EncodeParameters) -> EncoderResult<EncodeParameters> {
        match encinfo.format {
            NACodecTypeInfo::None => {
                ...
                ofmt.format =
                    NACodecTypeInfo::Video(NAVideoInfo::new(0, 0, self.flipped, YUV420_FORMAT));
                Ok(ofmt)
            }
            NACodecTypeInfo::Audio(_) => Err(EncoderError::FormatError),
            NACodecTypeInfo::Video(vinfo) => {
                let outinfo = NAVideoInfo::new(
                    (vinfo.width + 3) & !3,
                    (vinfo.height + 3) & !3,
                    self.flipped,
                    YUV420_FORMAT,
    ...
    fn init(&mut self, stream_id: u32, encinfo: EncodeParameters) -> EncoderResult<NAStreamRef> {
        match encinfo.format {
            ...
            NACodecTypeInfo::Video(vinfo) => {
                ...
                let out_info =
                    NAVideoInfo::new(vinfo.width, vinfo.height, self.flipped, vinfo.format);
                let info = NACodecInfo::new(
                    if self.flipped { "vp6" } else { "vp6f" },
                    NACodecTypeInfo::Video(out_info),
                    None,
                );
                let mut stream = NAStream::new(
                    StreamType::Video,
                    stream_id,
                    info,
                    encinfo.tb_num,
    ...
    fn encode(&mut self, frm: &NAFrame) -> EncoderResult<()> {
        ...
        if let Some(ref vbuf) = buf.get_vbuf() {
            ...
            let golden_frame = if is_intra {
                ...
                if !self.last_gold {
                    self.fenc.estimate_mvs(
                        self.gold_frame.get_vbuf().unwrap(),
                        self.mc_buf.clone(),
                        true,
                    );
                }
            ...
            self.fenc
                .reconstruct_frame(&mut self.dc_pred, self.last_frame.get_vbuf().unwrap());
...
const ENCODER_OPTS: &[NAOptionDefinition] = &[
    NAOptionDefinition {
        name: KEYFRAME_OPTION,
        description: KEYFRAME_OPTION_DESC,
        opt_type: NAOptionDefinitionType::Int(Some(0), Some(128)),
    },
    NAOptionDefinition {
        name: HUFFMAN_OPTION,
        description: "use Huffman encoding",
    ...
    NAOptionDefinition {
        name: QUANT_OPTION,
        description: "force fixed quantiser for encoding",
        opt_type: NAOptionDefinitionType::Int(Some(-1), Some(63)),
    },
    NAOptionDefinition {
        name: MV_SEARCH_OPTION,
        description: "motion search mode",
        opt_type: NAOptionDefinitionType::String(Some(&["full", "dia", "hex"])),
    },
    NAOptionDefinition {
        name: MV_RANGE_OPTION,
        description: "motion search range (in pixels)",
            ...
            for opt_def in ENCODER_OPTS.iter() {
                if opt_def.check(option).is_ok() {
                    match option.name {
                        ...
                        QUANT_OPTION => {
                            if let NAValue::Int(intval) = option.value {
                                self.force_q = if intval < 0 {
                                    None
                                } else {
                                    Some(intval as usize)
                                };
                            }
                        }
                        MV_SEARCH_OPTION => {
...
mod test {
    ...
    fn test_vp6_encoder_huff() {
        let enc_options = &[
            NAOption {
                name: super::HUFFMAN_OPTION,
                value: NAValue::Bool(true),
            },
            NAOption {
                name: super::QUANT_OPTION,
                value: NAValue::Int(42),
